...
**AIShield Watchtower** is designed to automate model and notebook discoveries and conduct comprehensive vulnerability scans.
...
AIShield Watchtower can inspect vulnerabilities in Jupyter notebooks and AI/ML Models (.h5, .pkl, and .pb file formats).

For using AIShield Watchtower, clone the Watchtower repo. You can just install prerequisites and scan your notebooks and AI/ML models. Some starting sample test files are available within the Watchtower repo.
...
- For running the UI-Docker version, docker and docker-compose should be installed in the host system
...
Once Git repositories are cloned, change the directory.
...
Install Watchtower-related dependency libraries using the following commands
...
To inspect artifacts present in the Public Github Repo

```python
python watchtower.py --repo_type=github --repo_url=<Enter Github Repo>
```

To scan the artifacts present in the AWS S3 bucket
...
open the browser and paste: <http://localhost:5015/watchtower-aishield>
...
For using Watchtower UI, build a docker image for Watchtower and run the Watchtower image
...
On successful completion of the scan, Watchtower vulnerability reports will be available in the reports folder in the Watchtower root folder.  

To stop and remove the Watchtower image execute the following
...
In the Watchtower Playground, users can scan Notebooks and AI/ML models available in public Github Repos by providing a public Github Repo URL. After completion of the vulnerability scan, reports will be available to download on the Playground screen.

## Reports

On successful completion of the Watchtower scan, three reports will be generated in the following path :

- For CLI Mode - all three reports will be available inside the Watchtower src folder. Users may refer last line of the summary report in the console for the complete path of the report location

- For UI Mode - all three will be reports will be available inside the Watchtower src folder. Users may refer to the success message on the UI to get the path of the report location

- For UI-Docker Mode - all three reports will be available inside the Watchtower reports folder. Users may refer to the success message on the UI to get the path of the report location

01. Summary Report - In summary report will provide information on the number of model files and notebook files detected, the number of vulnerabilities detected, and the Count of those vulnerabilities mapped to Critical, High, Medium, and Low. Sample snippet of Summary Report:
...
02. Severity Mapping Report - In this report, details of High, Medium, and Low mapping to Models or Notebooks are reported. Sample snippet of Severity Mapping Report:
...
- **Model and Notebook Detection**: Automatically recognizes AI/ML models and Notebooks within a provided repository.
  The supported file format is H5, pickle, saved model, .ipynb
...
4. Presidio analyzer has 1000000 (1GB) as max length. Any data greater than 1GB will not be captured in reports.  
